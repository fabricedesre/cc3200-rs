...
fn get_update(filename: &str, url: &str) -> Result<(), Error> {
    ...
    let file_length = try!(content_length(&response.headers));
    let mut file = try!(File::create(filename, file_length, true));
...
fn update_board() -> Result<(), Error> {
    ...
    match get_update(filename, OTA_URL) {
        Ok(_) => {
            // nothing to do
        },
        Err(err) => {
    ...
    try!(apply_update(filename, imagename));
